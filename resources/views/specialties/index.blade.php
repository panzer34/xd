...
<a href="" class= "btn btn-primary btn-circle" data-toggle="modal" data-target="#createMdl" data-keyboard="false" data-backdrop="static">
...
@include('specialties.modal.create')
            ...
            <table id="especialidad" class="table table-bordered display nowrap" cellspacing="0" width= "100%">
                ...
                @foreach($specialties as $specialty)
                    <tr class="text-center">
                        ...
                        <td>
                        <input data-id="{{$specialty->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="InActive" {{ $specialty->status ? 'checked' : '' }}>
                        </td>
                        ...
                        <td>
                            <a href="#" data-toggle="modal" data-target="#editMdl-{{$specialty->id}}" data-keyboard="false" data-backdrop="static" >
                        ...
                        @include('specialties.modal.update')
                        @include('specialties.modal.delete')
                </tr>
                @endforeach
...
@if(!$errors->isEmpty())
    ...
    <script>
        $(function(){
            $('#editMdl-{{$specialty->id}}').modal('show');
        });
...
$(function() {
    $('.toggle-class').change(function() {
        ...
        $.ajax({
            ...
            dataType: "json",
            url: 'changeStatusb',
            data: {'status': status, 'specialty_id': specialty_id},
